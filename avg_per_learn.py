...
class AvgLearn(object):
    def __init__(self):
        ...
        self.avgWtDict = defaultdict(lambda: 0)  # contains the avg weight of eveyr word
        self.avgBias = 0    # avg bias
        self.counter = 1
            ...
            for sample in self.allFiles:
                ...
                if (y * alpha <= 0):
                    for word in wordlist:
                        ...
                        self.avgWtDict[word] += y * self.counter
                        self.avgBias += y * self.counter

                self.counter += 1

        for k, v in self.avgWtDict.items():
            self.avgWtDict[k] = self.weightDict[k] - ((1/self.counter) * self.avgWtDict[k])
